...
func TestCLIStreamUploadsAndSendsAttachment(t *testing.T) {
	path := filepath.Join(t.TempDir(), "Image1.jpg")
	if err := os.WriteFile(path, []byte("image bytes"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	server = httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		switch request.URL.Path {
		case config.PresignedURLPath:
			_, _ = writer.Write([]byte(`{"code":"000000","message":"success","data":{"upload_url":"` + server.URL + `/upload/object?signature=temporary","public_url":"https://public.example.test/chat/Image1.jpg","method":"PUT","required_headers":{"Content-Type":"image/jpeg"}}}`))
		case "/upload/object":
			writer.WriteHeader(http.StatusNoContent)
		case config.StreamPath:
			if got := request.Header.Get("Authorization"); got != "Bearer test-token" {
				t.Fatalf("stream Authorization = %q", got)
			}
			var body api.StreamRequest
			if err := json.NewDecoder(request.Body).Decode(&body); err != nil {
				t.Fatal(err)
			}
			if body.ConversationID != "conversation-1" || body.Prompt != "头发改为红色" || body.Mode != "design" {
			...
			if len(body.Files) != 1 || body.Files[0] != (api.ChatAttachment{
				...
				t.Fatalf("stream files = %#v", body.Files)
			}
	...
	root.SetArgs([]string{"stream", "--conversation-id", "conversation-1", "--prompt", "头发改为红色", "--file", path})
	if err := root.Execute(); err != nil {
		t.Fatal(err)
	}
