...
不执行积分估算，遇到计费信息也不暂停。用户已经明确要求创作或继续整部短剧时，若服务端 `review` 或 `billing_pending` 只是在询问是否进入下一生成阶段，使用服务端原始“继续/确认”选项和同一个 `conversation_id` 推进，不自行改写选项值。

如果服务端要求选择剧情、风格、角色、镜头、模型或其他会改变创作结果的内容，仍应展示原始问题与选项并等待用户决定；不要把创作选择当成纯阶段确认自动代答。
